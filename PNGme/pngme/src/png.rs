...
impl Png {
    ...
    pub fn remove_chunk(&mut self, chunk_type: &str) -> Result<Chunk, ChunkTypeError> {
    ...
    pub fn as_bytes(&self) -> Vec<u8> {
        ...
        for i in 0..self.chunks.len() {
            bytes.extend_from_slice(&self.chunks[i].as_bytes());
        }
        bytes
    }
}
...
impl std::convert::TryFrom<&[u8]> for Png {
    ...
    fn try_from(input: &[u8]) -> Result<Self, Self::Error> {
        //compare to see if first 8 match header
        let header: [u8;8] = input[0..8].try_into().unwrap();
        if header != Png::STANDARD_HEADER {
            return Err(ChunkTypeError::InvalidArgs { details: "Not PNG header!".to_string() } );
        }
        let mut chunks: Vec<Chunk> = Vec::new();

        //take length of first chunk, create slice by using length of chunk + the 12 bytes for type, length, and crc
        //push through the try_from for chunk creation. Add chunk to chunk vector
        let mut current_index = 8;
        while current_index < input.len()-1 {
            let mut length: Vec<u8> = Vec::new();
            for i in current_index..current_index+4 {
                length.push(input[i]);
            }
            let length = u32::from_be_bytes(length.try_into().unwrap());
            let total: usize = (length + 12).try_into().unwrap();
            chunks.push(Chunk::try_from(&input[current_index..current_index+total]).unwrap());
...
impl std::fmt::Display for Png {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        ...
        for i in 0..self.chunks.len() {
            message.push_str(&self.chunks[i].chunk_type().to_string());
            message.push_str(" ");
        }
        Ok(println!("{}",message))
    }
